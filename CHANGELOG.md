...
- **되돌리기 / 다시 실행 (⌘Z, ⇧⌘Z)** — 텍스트 편집·설정 변경·하이라이트를 하나의
  시간순 히스토리로 묶어, 한 단축키로 앞뒤를 오갈 수 있습니다. Windows/Linux 는
  Ctrl+Z, Ctrl+Shift+Z, Ctrl+Y 도 받습니다. 텍스트 단계는 CodeMirror 에 위임하므로
  타이핑 단위감은 그대로입니다.

### 수정

- 언어를 바꾸면 에디터가 통째로 새로 만들어지면서 그때까지의 텍스트 되돌리기가
  사라지던 문제. 언어에 묶여 있던 `key` 를 떼고, mode 를 CodeMirror 옵션 변경으로
  넘겨 같은 인스턴스를 유지합니다.
